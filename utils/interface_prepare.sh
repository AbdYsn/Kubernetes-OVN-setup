...
while test $# -gt 0; do
  case "$1" in
   ...
   --help | -h)
      echo "
interface_prepare.sh -i <interface> [options]: A script to configure an interface using the network service.

options:
 
	--interface | -i) <interface>		The name of the interface to be configured.
   
	--ip) <wanted ip>			The ip of the master node.

	--netmask) <netmask>			The netmask to be configured on the interface.
	...
	--no-restart)				Disable the restart of the network service after configuring the ifcfg file.
...
if [[ -z "$interface" ]]
then
   echo "The interface name was not provided !!!
...
then
   if [[ -z "$bond_master" ]]
   then
      echo "The interface s configured as bond slave interface, but no master specified, please use --bond-master option to provide one."
      echo "Exiting...."
      exit 1
   fi
elif [[ "$bond" != "master" ]]
then
   echo "Unknown bond option $bond!!, please either use: master, or slave options."
   echo "Exiting...."
   exit 1
fi
...
if [[ "$bond" == "master" ]]
then
   bond_master_configs "$bond_mode" "$bond_miion" "$bond_updelay" "$bond_downdelay"
fi

if [[ "$bond" == "slave" ]]
then
   bond_slave_configs "$interface" "$bond_master"
fi
